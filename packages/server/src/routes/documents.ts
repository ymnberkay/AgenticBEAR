...
export async function documentRoutes(app: FastifyInstance): Promise<void> {
  app.get<{ Params: { projectId: string } }>('/api/projects/:projectId/documents', async (request, reply) => {
    return reply.send(documentRepo.findByProjectId(request.params.projectId));
  });
  ...
  app.post<{ Params: { projectId: string }; Body: CreateProjectDocumentInput }>(
    ...
    async (request, reply) => {
      ...
      return reply.status(201).send(documentRepo.create(request.params.projectId, { name, content }));
    },
  );

  app.delete<{ Params: { id: string } }>('/api/documents/:id', async (request, reply) => {
    const removed = documentRepo.remove(request.params.id);
