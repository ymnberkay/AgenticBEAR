...
import type { Agent, Message, Plan, PlanStep, TaskResult } from '../types';
import type { LMBridge } from '../lm/LMBridge';
import { FileTools } from '../tools/FileTools';
import { agentStore } from '../agents/AgentStore';

export type OrchestratorEvent =
  | { type: 'plan'; plan: Plan }
  | { type: 'step_start'; step: PlanStep; index: number; total: number }
  | { type: 'step_done'; result: TaskResult }
  | { type: 'tool_call'; toolName: string; path?: string }
  | { type: 'chunk'; agentName: string; text: string }
  | { type: 'error'; message: string }
  | { type: 'done'; results: TaskResult[] };
...
export class Orchestrator {
  constructor(private lm: LMBridge) {}
  ...
  async run(
    ...
    let planText = '';
    try {
      const res = await this.lm.send(orchestratorAgent, planMessages, [], token,
    ...
    for (let i = 0; i < plan.steps.length; i++) {
      ...
      const result = await this.runStep(
        agent, step.task, contextSummary.join('\n'), fileTools, token, onEvent,
      );

      results.push(result);
      contextSummary.push(
        `${agent.name} completed: ${step.task}\n` +
  ...
  private async runStep(
    ...
    while (round < MAX_ROUNDS) {
      ...
      let roundText = '';
      try {
        const res = await this.lm.send(agentWithTools, messages, [], token,
          chunk => {
            roundText += chunk;
            // Stream visible text (without tool call blocks) to chat
            const visible = stripToolCalls(chunk);
            if (visible) onEvent({ type: 'chunk', agentName: agent.name, text: visible });
          },
        );
      ...
      for (const tc of toolCalls) {
        ...
        const result = await fileTools.execute(tc.tool, tc as unknown as Record<string, unknown>);
  ...
  private parsePlan(text: string, specialists: Agent[]): Plan {
    try {
      const match = text.match(/\{[\s\S]*\}/);
      if (!match) throw new Error('no JSON');
      const raw = JSON.parse(match[0]) as { summary?: string; steps?: PlanStep[] };
      return {
