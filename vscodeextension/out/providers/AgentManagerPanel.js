...
const agentTemplates_1 = require("../agents/agentTemplates");
class AgentManagerPanel {
    static createOrShow(extensionUri, lm) {
        if (AgentManagerPanel._current) {
            AgentManagerPanel._current._panel.reveal();
            return AgentManagerPanel._current;
        }
        const panel = vscode.window.createWebviewPanel('agenticbear.agentManager', 'AgenticBEAR — Agents', vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
        AgentManagerPanel._current = new AgentManagerPanel(panel, lm);
        return AgentManagerPanel._current;
    }
    constructor(panel, lm) {
        this._panel = panel;
        this._lm = lm;
        ...
        this._panel.webview.onDidReceiveMessage(async (msg) => {
            switch (msg.type) {
                ...
                case 'requestModels':
                    ...
                    break;
                case 'saveAgent':
                    await this.saveAgent(msg.agent);
                    break;
                case 'deleteAgent':
                    await AgentStore_1.agentStore.remove(msg.id);
                    await this.sendAgents();
                    break;
            }
        });
        AgentStore_1.agentStore.onDidChange(async () => this.sendAgents());
    }
    async sendAll() {
        await this.sendAgents();
        await this.sendModels();
        this._panel.webview.postMessage({ type: 'templates', categories: agentTemplates_1.TEMPLATE_CATEGORIES });
    }
    async sendAgents() {
    ...
    async saveAgent(data) {
        if (data.id) {
            await AgentStore_1.agentStore.update(data.id, data);
        }
        else {
            await AgentStore_1.agentStore.add({
                name: data.name,
                role: data.role,
